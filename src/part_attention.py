...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, end2end=True):
        ...
        self.chuan_fc = nn.Sequential(nn.Linear(3, 1),
                                   nn.Sigmoid())
        self.fc_regression = nn.Sequential(nn.Linear(1024, 1),
                                   nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        # pdb.set_trace()
        #regression_score = self.fc_regression(output)
        # regression_score = F.dropout(regression_score, p=0.5)
        pred_score = self.fc(output)
        # pred_score = F.dropout(pred_score, p=0.5)
        regression_score = self.chuan_fc(pred_score)
        paraller_regression_score = self.fc_regression(output)
        # pred_score = F.dropout(pred_score, p=0.5)
        #x = x.view(x.size(0), -1)



        return (regression_score+paraller_regression_score)/2, pred_score, output
